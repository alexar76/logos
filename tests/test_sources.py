...
async def test_hub_source_uses_public_v2_federation_route(monkeypatch) -> None:
    ...
    assert snapshot["healthy_hubs"] == 2
    assert snapshot["total_capabilities"] == 4


@pytest.mark.asyncio
async def test_hub_source_treats_active_peers_as_healthy(monkeypatch) -> None:
    """Prod Hub lists peers as status=active, not online — must not read as 1/N healthy."""

    class _ActivePeersClient(_HubClient):
        async def get(self, url: str) -> _Response:
            self.urls.append(url)
            if url.endswith("/ai-market/v2/federation/peers"):
                return _Response({
                    "peers": [
                        {"url": "https://iot.modelmarket.dev", "name": "gaia", "status": "active"},
                        {"url": "https://peer-b.example", "name": "b", "status": "active"},
                    ],
                })
            if url.endswith("/ai-market/v2/manifest"):
                return _Response({"total_capabilities": 10, "local_capabilities": 2, "federated_capabilities": 8})
            return _Response({}, 404)

    _ActivePeersClient.urls = []
    monkeypatch.setattr("logos.sources.hub.httpx.AsyncClient", _ActivePeersClient)

    snapshot = await HubSource("https://modelmarket.dev").fetch_snapshot()

    assert snapshot["total_hubs"] == 3
    assert snapshot["healthy_hubs"] == 3
    assert all(p["healthy"] for p in snapshot["peers"])
